...
import { addCredByToken } from '../../../../Services/userSkService';
const ClientId = () => {
  let [copyClient, setCopyClient] = useState(false);
  let [copyClientSecret, setCopyClientSecret] = useState(false);
  let [allCred, setAllCred] = useState([]);
  let copyClientIdFun = () => {
    setTimeout(() => {
      setCopyClient(false)
    }, 5000);
    let client_id = document.getElementById('client_id');
    let client_id_text = client_id.innerText;
    navigator.clipboard.writeText(client_id_text)
  }
  let copyClientSecretFun = () => {
    setTimeout(() => {
      setCopyClientSecret(false)
    }, 5000);
    let client_secret = document.getElementById('client_secret');
    let client_secret_text = client_secret.innerText;
    navigator.clipboard.writeText(client_secret_text)
  }

  let createNewCredFun = async () => {
    let token = localStorage.getItem("1001_ai_console_token")
    let resultCred = await addCredByToken(token);
    setAllCred([...allCred, resultCred?.data])
  }
  return (
    <>
        {/* Page Wrapper */}
      ...
      <div className="grid place-items-center">
        ...
        <div className='text-center' style={{display : allCred?.length === 0 ? '' : 'none'}}>
        <i className="fa-solid fa-times text-7xl"></i>
        <h2 class="text-2xl text-center">No Credentials Created</h2>
        <button className="btn bg-slate-150 mt-2 font-medium text-slate-800 hover:bg-slate-200 focus:bg-slate-200 active:bg-slate-200/80 dark:bg-navy-500 dark:text-navy-50 dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90" onClick={createNewCredFun}>
                  Create
                </button>
        </div>
          <div style={{display : allCred?.length !== 0 ? '' : 'none'}}>
          <button className="btn border my-2 border-info font-medium text-info hover:bg-info hover:text-white focus:bg-info focus:text-white active:bg-info/90" onClick={createNewCredFun}>
                  Add Credentials
                </button>
            {allCred?.map((cred) => (
          ...
          <div className="flex items-center py-4">
                  <h1 className="text-3xl text-primary dark:text-accent-light pr-3" id='client_id'>{cred?.client_id}
                </h1>
                <a href='javascript:void(0);' onClick={()=> {
                  copyClientIdFun();
                  setCopyClient(true)
                }}>
                 {copyClient === false ? (<i className="fa-solid fa-copy text-lg"></i>) : (<i className="fa-solid fa-check text-lg text-primary"></i>)}
                </a>
          </div>
                  <span><b>Client Secret</b></span>
          <div className="flex items-center py-4">
                  <h1 className="text-3xl text-primary dark:text-accent-light pr-3" id='client_secret'>{cred?.client_secret}
                </h1>
                <a href='javascript:void(0);' onClick={()=> {
                  copyClientSecretFun()
                  setCopyClientSecret(true)
              }}>
                {copyClientSecret === false ? (<i className="fa-solid fa-copy text-lg"></i>) : (<i className="fa-solid fa-check text-lg text-primary"></i>)}
                </a>
          </div>
              </div>
            ))}
