...
import {allNotifications} from '../../Content/Notifications/AllNotifications'
const Header = () => {
  let [dark_mode, setDark_mode] = useState(false)
  let [monochrome_mode, setMonochrome_mode] = useState(false)
  let [menu, setMenu] = useState(true)
  let [notifications, setNotifications] = useState(false)

  useEffect(() => {
    const savedDarkMode = localStorage.getItem("_x_darkMode_on");
    const isDarkMode = savedDarkMode === "true";
    setDark_mode(JSON.parse(isDarkMode));
  }, []);
  const localStorageFun = () => {
    const newDarkMode = !dark_mode;
    setDark_mode(newDarkMode);
    localStorage.setItem("_x_darkMode_on", newDarkMode.toString());
    window.location.reload()
  };
  const localStorageMonochromeFun = () => {
    const newMonochromeMode = !monochrome_mode;
    setMonochrome_mode(newMonochromeMode);
    localStorage.setItem("_x_MonochromeMode_on", newMonochromeMode.toString());
    window.location.reload()
  };
  const toggleMenuFun = () => {
    // Retrieve the current value from localStorage
    const currentValue = localStorage.getItem("sidebar_on");
  
    // Toggle the value (from true to false or vice versa)
    const newToggle = currentValue === "true" ? "false" : "true";
  
    // Update localStorage with the new value
    localStorage.setItem("sidebar_on", newToggle);
  
    // Reload the page (or you can choose to do this conditionally)
    window.location.reload();
  };
  
  useEffect(() => {
    const savedMonochromeMode = localStorage.getItem("_x_MonochromeMode_on");
    const isMonochromeMode = savedMonochromeMode === "true";
    setMonochrome_mode(JSON.parse(isMonochromeMode));

    const savedSidebarToggle = localStorage.getItem("sidebar_on");
    const isSidebarToggle = savedSidebarToggle === "true";
    setMenu(JSON.parse(isSidebarToggle));
  }, []);
...
<div className="header-container relative flex w-full bg-white dark:bg-navy-700 print:hidden">
  ...
  <div className="flex w-full items-center justify-between">
    ...
    <div className="h-7 w-7">
      <button className={`menu-toggle ${menu === true ? 'active' : ''} ml-0.5 flex h-7 w-7 flex-col justify-center space-y-1.5 text-primary outline-none focus:outline-none dark:text-accent-light/80`} onClick={toggleMenuFun}>
        <span />
        <span />
        <span />
      </button>
    ...
    <div className="-mr-1.5 flex items-center space-x-2">
      ...
      <button className="btn h-8 w-8 rounded-full p-0 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25 sm:hidden">
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5.5 w-5.5 text-slate-500 dark:text-navy-100" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
          <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
        </svg>
      </button>
      {/* Main Searchbar */}
      <template x-if="$store.breakpoints.smAndUp" /><div className="flex" x-data="usePopper({placement:'bottom-end',offset:12})">
        <div className="relative mr-4 flex h-8">
          <input placeholder="Search here..." className="form-input peer h-full rounded-full bg-slate-150 px-4 pl-9 text-xs+ text-slate-800 ring-primary/50 hover:bg-slate-200 focus:ring dark:bg-navy-900/90 dark:text-navy-100 dark:placeholder-navy-300 dark:ring-accent/50 dark:hover:bg-navy-900 dark:focus:bg-navy-900 w-60" type="text" x-ref="popperRef" />
        ...
        <div x-ref="popperRoot" style={{position: 'fixed', inset: '0px 0px auto auto', margin: '0px', transform: 'translate(-234px, 58px)', display : 'none'}} data-popper-placement="bottom-end">
          <div className="popper-box flex max-h-[calc(100vh-6rem)] w-80 flex-col rounded-lg border border-slate-150 bg-white shadow-soft dark:border-navy-800 dark:bg-navy-700 dark:shadow-soft-dark">
            <div x-data="{activeTab:'tabAll'}" className="is-scrollbar-hidden flex shrink-0 overflow-x-auto rounded-t-lg bg-slate-100 px-2 text-slate-600 dark:bg-navy-800 dark:text-navy-200">
      ...
      {/* Dark Mode Toggle */}
      <button className="btn h-8 w-8 rounded-full p-0 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25" onClick={localStorageFun}>
      {dark_mode === true ? (        <svg className="h-6 w-6 text-amber-400" fill="currentColor" viewBox="0 0 24 24">
      ...
      <button className="btn h-8 w-8 rounded-full p-0 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25" onClick={localStorageMonochromeFun}>
      ...
      <div x-effect="if($store.global.isSearchbarActive) isShowPopper = false" x-data="usePopper({placement:'bottom-end',offset:12})" className="flex">
        <button x-ref="popperRef" className="btn relative h-8 w-8 rounded-full p-0 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25" onClick={()=> setNotifications(!notifications)}>
        ...
        <div className={`popper-root ${notifications && 'show'}`} x-ref="popperRoot" style={{position: 'fixed', inset: '0px 0px auto auto', margin: '0px', transform: 'translate(-98px, 58px)'}} data-popper-placement="bottom-end">
          <div x-data="{activeTab:'tabAll'}" className="popper-box mx-4 mt-1 flex max-h-[calc(100vh-6rem)] w-[calc(100vw-2rem)] flex-col rounded-lg border border-slate-150 bg-white shadow-soft dark:border-navy-800 dark:bg-navy-700 dark:shadow-soft-dark sm:m-0 sm:w-80">
            ...
            <div className="tab-content flex flex-col overflow-hidden">
              <div className="is-scrollbar-hidden space-y-4 overflow-y-auto px-4 py-4" style={{overflowY : 'auto', height : '220px'}}>
                {allNotifications?.map((notifications, index) =>{
                  return (
                  <>
                  {notifications.description ? (                  <div className="flex items-center space-x-3">
                  <div className={notifications.background}>
                    {/* <img src={notifications.icon} className={notifications.iconColor} /> */}
                    {index === 1  &&(<svg xmlns="http://www.w3.org/2000/svg" class='h-5 w-5 text-info' fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>)}
                    {index === 4  &&(<svg xmlns="http://www.w3.org/2000/svg" class='h-5 w-5 text-info' fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>)}
                    {index === 6 &&(<svg xmlns="http://www.w3.org/2000/svg" class='h-5 w-5 text-warning' fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
        <path strokeLinecap="round" strokeLinejoin="round" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" />
      </svg>)}
                  </div>
                  <div>
                    <p className="font-medium text-slate-600 dark:text-navy-100 text-align-left fw-650">
                    {notifications.heading}
                    </p>
                    <div className="mt-1 flex text-xs text-slate-400 dark:text-navy-300">
                      <span className="shrink-0 text-align-left fw-600">{notifications.para}</span>
                      <div className="mx-2 my-1 w-px bg-slate-200 dark:bg-navy-500" />
                      <span className="line-clamp-1 text-align-left fw-600">{notifications.description}</span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="mt-1 text-xs text-slate-400 line-clamp-1 dark:text-navy-300 text-align-left fw-600">
                    {notifications.para}
                    </div>
                  </div>
                </div>)}
                ...
                {/* <div className="flex items-center space-x-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-info/10 dark:bg-info/15">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-info" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                  </div>
                  <div>
                ...
                {/* <div className="flex items-center space-x-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-warning/10 dark:bg-warning/15">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-warning" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" />
                    </svg>
                  </div>
                  <div>
              ...
              <div className="is-scrollbar-hidden space-y-4 overflow-y-auto px-4 py-4" style={{display: 'none'}}>
                <div className="flex items-center space-x-3">
              ...
              <div className="is-scrollbar-hidden overflow-y-auto px-4" style={{display: 'none'}}>
